...
7. **Write and submit real WhatsApp template content for attempts 2-4, AND create the matching Pabbly campaigns** — `review_request_v2/3/4` (templates) and `review_2/3/4` (Pabbly campaigns) are currently just placeholder names in the database with nothing real behind either one. Only attempt 1 (`review_request_v1` template, `review` campaign) actually works today. The 2026-08-24 session's validation guards will now correctly block a send with a clear error naming the missing attempt, rather than letting a broken send reach Pabbly — but the underlying setup (write + Meta-approve 3 templates, create 3 Pabbly campaigns each with its template selected) is still not done. Email attempts 2-4 already work.
7a. **Set a real `review_request_link` for every business, including Print Express** — found during the 2026-08-24 accuracy pass: this column is `null` for all 11 businesses. Nothing will actually send until this is set, by the new validation guards' design (better than silently emailing a dead button link).
7b. **Create the real Pabbly HTTP Header Auth credential in n8n** and attach it to `send-review-request.json`'s "HTTP Request" node — still the placeholder ID `REPLACE_WITH_YOUR_PABBLY_CHATFLOW_CREDENTIAL_ID`.
