...
### 1. Outbound "send review request" workflow — ✅ built, corrected, and extended with follow-up scheduling
File: `n8n/send-review-request.json` (exported from n8n as "Google GBP Send Review Request", edited here, needs re-import)
...
**Both "Log ... Send" queries** (one per channel; `notes` records which attempt number was sent, and for email, the actual subject used):
```sql
insert into activities (
  business_id, customer_id, activity_type, activity_status, channel,
  template_name, personalized_image_url, attempts, notes
) values (
  '{{ $json.business_id }}', '{{ $json.customer_id }}', 'Request Sent', 'Sent',
  'Email', '{{ $json.email_subject }}', '{{ $json.project_image_url }}',
  {{ $json.attempt_count + 1 }}, 'Sent via n8n email workflow — attempt {{ $json.attempt_count + 1 }}'
)
returning id;
```
(WhatsApp version identical shape, `channel = 'WhatsApp'`, `template_name = '{{ $json.whatsapp_template }}'`.)
...
**Channel branching logic**: branch on `customers.phone` (not `businesses.whatsapp_sender_number` — that field is per-business, identical across all of a business's customers, so it can't distinguish which *customers* have WhatsApp). The IF node's branches are now actually wired: true (has phone) → Pabbly Chatflow HTTP Request, false → Gmail.
...
**Still open**: the workflow itself hasn't been live-tested end-to-end since this restructuring (only the previous Form Trigger version was live-tested) — needs re-import (or update) in n8n with credentials re-attached, then a real submission from the website with and without a photo.
...
### 3. Inbound GBP review capture, AI reply, and business notification — ✅ built and tested (capture portion confirmed with real data; reply/notify extension built, not yet live-tested)
File: `n8n/inbound-gbp-review-capture.json` (25 nodes)

**Full chain:**
```
Schedule Trigger (6h) → Get Active Businesses with GBP (Postgres) → List Reviews for Location (GBP API v4)
  → Flatten Reviews (word rating → number) → Keep Review Data → Check Already Logged (Postgres)
  → Merge Review + Check Result → New Review? (IF: existing_status != 'Completed')
      → Needs Insert or Just Retry? (IF: existing_activity_id empty?)
          ├─ true (no row yet)  → Find Best Match Customer → Merge Match Result → Insert Review Activity (status='Pending')
          └─ false (row exists, still Pending) → Prep Retry (Skip Insert)
      → [both converge] Attach Activity ID → Good Review? (star_number >= 4)
          ├─ true (4-5★)  → Generate AI Reply (Gemini text) → Extract Reply Text → Post Reply to GBP
          │                   → Log Reply on Activity (sets status='Completed') → Get Business Contact Email → Merge Email
          └─ false (1-3★) → Flag Low-Star for Manual Reply (appends note, no auto-reply)
                              → Get Business Contact Email → Merge Email
      → [both converge] Notify Business of New Review (Gmail, sends to businesses.contact_email)
```
...
- Calls Gemini (`gemini-2.5-flash`, text model — not the image model used elsewhere) with the reviewer's name, star rating, and actual review comment; asks for a genuine 2-3 sentence reply referencing what they said, not a generic template.
...
- **Not addressed**: checking GBP's own `reviewReply` field (present in the API response) to detect a reply that already exists on Google itself (e.g. posted manually before this workflow existed, or on a listing with prior history) — deprioritized by the user, worth revisiting before onboarding a business with existing review history.
...
**Lesson for future n8n workflows**: avoid `$('NodeName').item` when a branch may carry multiple items — prefer keeping data on `$json` of the current item, or use `.all()`/`.first()` explicitly with manual index-based re-pairing when merging results from a parallel lookup (e.g. a Postgres lookup run once per item).

**Setup still needed before the extended (reply+notify) portion can be tested live:**
- Reselect Postgres credential on all Postgres nodes (placeholder IDs)
- Reselect Google OAuth2 credential (already connected in n8n) on "Post Reply to GBP"
- Attach `geminiApi` Header Auth credential on "Generate AI Reply"
- Add/select a Gmail OAuth2 credential on "Notify Business of New Review" (can reuse the one from workflow #2 if same sending account)
...
- **"email rate limit exceeded" hit during testing (this session)**: not a bug — Supabase's built-in test mailer (no custom SMTP configured; `smtp_host` is null on this project) caps auth emails at **`rate_limit_email_sent: 2` per hour, project-wide** (not per-recipient — checked via the Management API, `GET /v1/projects/xbmoomufmxicwymwbovg/config/auth`), plus a 60-second minimum gap between sends (`smtp_max_frequency: 60`). Repeated login attempts across two test emails in quick succession used up the hour's quota. **User chose to just wait it out for now** rather than configure custom SMTP immediately — but this WILL become a real blocker before onboarding actual businesses, since 2 logins/hour total (across every business, every staff member) is far too low for live usage. **Before going live**: configure a real SMTP provider (e.g. Gmail SMTP, SendGrid, Resend, Postmark) under Supabase dashboard → Authentication → Emails → SMTP Settings — this removes the low built-in cap entirely.
...
- Not yet deployed anywhere real (Netlify/Vercel/etc.) — still local-only. User said they'll build "a proper website" incorporating this login next, and confirmed the migration will be easy since the page has no dependency on its hosting URL beyond the Supabase Redirect URL whitelist and the (self-adapting) `emailRedirectTo` value.
...
- **Process improvement adopted**: as of the `pg_trgm`/placeholder-customer setup, the user switched to storing the Supabase personal access token in the project's `.env` file (`c:\Users\Jonas Mohlala\Dynamic\.env`, key `SUPABASE_ACCESS_TOKEN`) rather than pasting it directly in chat each time — Claude reads it from there instead. Better practice going forward for any credential, not just Supabase. Note: this `.env` file also holds unrelated live secrets (Anthropic, LinkedIn, Gemini, Google API keys) from other projects — no `.gitignore` exists yet and this isn't a git repo, so not an urgent exposure risk today, but should be addressed (add `.gitignore` covering `.env`) before this project is ever pushed to a remote repository.
...
Everything core is now built. Remaining open items, roughly in priority order:

1. **Import/update `n8n/customer-intake-form.json` in n8n** (Webhook-triggered version, replaces the old Form Trigger version; Production URL `https://n8n.ai4smartautomations.com/webhook/customer-intake` already wired into `web/add-customer.html`), attach the `geminiApi` Header Auth credential, the Postgres credential, and the Supabase service-role credential referenced in "Upload Photo to Storage". Live-test end-to-end (with and without a photo, and one deliberate failure case e.g. missing business_id) — this restructured version hasn't been tested yet, only the old Form Trigger shape was.
2. **Write and submit real WhatsApp template content for attempts 2-4** (`review_request_v2/3/4`) for Meta approval — currently just placeholder names in the database with no real message content. Only attempt 1 (`review_request_v1`) actually sends via WhatsApp today; attempts 2-4 will silently fail or send nothing meaningful until this is done. Email attempts 2-4 already work.
3. **Live-test the new `web/customers.html` and `web/settings.html` pages** (both built this session, neither opened in a browser yet) — sign in, confirm the customer list loads and the status dropdown correctly updates `opt_out`/`do_not_call`, and confirm Settings loads/saves `businesses` contact fields + status correctly.
4. **Finish wiring credentials on workflow #3's reply/notify extension** (Postgres, Google OAuth2 on "Post Reply to GBP", `geminiApi` on "Generate AI Reply", Gmail on "Notify Business of New Review") and live-test it — the capture portion works, but the AI-reply + notification extension hasn't been confirmed working end-to-end yet.
5. **Configure custom SMTP for Supabase Auth** before onboarding real businesses — currently on Supabase's built-in test mailer, capped at 2 auth emails/hour project-wide (`rate_limit_email_sent: 2`), which will break login for any real usage beyond light solo testing. **User has decided on Resend** as the provider — set via Supabase dashboard → Authentication → Emails → SMTP Settings when deploying.
6. **Deploy the website somewhere real** (Netlify, Vercel, or wherever the user's "proper website" ends up living) instead of the local Python dev server — user confirmed intent to do this next, said the migration should be easy since the page has no real hosting dependency beyond the Supabase Redirect URL whitelist.
6. **Rotate exposed credentials before onboarding real client data**: Supabase secret/service-role key, Gemini API key, Pabbly Chatflow API key, and the Postgres database password were all at some point visible in this chat transcript or an n8n export. Low urgency while only Print Express test data exists; treat as a pre-launch checklist item.
7. Formalize the ad-hoc `ALTER TABLE` changes (`review_request_link`, `project_image_url`) into tracked migration files if full reproducibility from `supabase/migrations/` matters later.
8. **Visual design pass on the web app** (colors, layout, etc.) — user has explicitly deferred this; functionality is prioritized first. Revisit once the functional checklist above is further along.
9. Optional: build a self-service "invite staff" flow on `settings.html` so linking additional users to a business doesn't require a manual SQL insert into `business_members` (see "Unassigned users, linking to a business" section above).
