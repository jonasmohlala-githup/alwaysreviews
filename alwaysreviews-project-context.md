...
- **"email rate limit exceeded" hit during testing, twice across two sessions** — not a bug. Supabase's built-in test mailer caps auth emails at **`rate_limit_email_sent: 2` per hour, project-wide** (not per-recipient), plus a 60-second minimum gap between sends (`smtp_max_frequency: 60`). First hit: user chose to wait it out. **Second hit (2026-08-20), resolved for real**: switched to **Resend** as custom SMTP, per the earlier-flagged decision (see `[[alwaysreviews_resend_smtp_todo]]` memory file). Configured via the Supabase Management API (`PATCH /v1/projects/xbmoomufmxicwymwbovg/config/auth`) rather than the dashboard: `smtp_host: smtp.resend.com`, `smtp_port: 465`, `smtp_user: resend`, `smtp_pass: <Resend API key>`, `smtp_sender_name: "Always Reviews"`, `smtp_admin_email: onboarding@resend.dev`. **Important caveat**: no domain was verified on Resend yet at setup time (`GET /domains` returned empty) — `alwaysreviews.com` needs to be added and DNS-verified on Resend before switching the from-address away from the shared `onboarding@resend.dev` test address. Until then, Resend's test mode only delivers to the Resend account's own verified email (confirmed working: `jonas.mohlala@gmail.com`, the account owner's email) — **not to real business customers/staff**. This is a hard blocker before onboarding any real business beyond solo testing. Confirmed working end-to-end: login → magic link email arrives via Resend → click → lands on live dashboard.
...
Updated 2026-08-20. Done since the last pass: workflow #3 fully live-tested end-to-end (capture → match → reply → notify, plus new GBP-fetch error handling and an "already replied on Google?" guard), several real bugs fixed (see workflow #3 section), app renamed to Always Reviews, deployed to GitHub + Vercel, Resend SMTP configured and confirmed working for login. Remaining open items, roughly in priority order:

1. **Verify `alwaysreviews.com` on Resend** (once purchased) and switch the SMTP from-address from `onboarding@resend.dev` to a real address on that domain (e.g. `noreply@alwaysreviews.com`) — hard blocker before onboarding any real business, since test mode only delivers to the Resend account's own verified email today.
...
6. **Fix the same `.first()` bug pattern if/when a second business is onboarded** — this session fixed `.first()` reach-backs to "Get Active Businesses with GBP" in "Flatten Reviews"/"Post Reply to GBP"/"Notify Business of New Review", but didn't audit `send-review-request.json` or `customer-intake-form.json` for the same issue; worth a pass before a second business goes live on any workflow.
7. **Import/update `n8n/customer-intake-form.json` in n8n** (Webhook-triggered version; Production URL `https://n8n.ai4smartautomations.com/webhook/customer-intake` already wired into `web/add-customer.html`), attach the `geminiApi` Header Auth credential, the Postgres credential, and the Supabase service-role credential referenced in "Upload Photo to Storage". Live-test end-to-end (with and without a photo, and one deliberate failure case e.g. missing business_id) — this restructured version still hasn't been tested, only the old Form Trigger shape was.
8. **Write and submit real WhatsApp template content for attempts 2-4** (`review_request_v2/3/4`) for Meta approval — currently just placeholder names in the database with no real message content. Only attempt 1 (`review_request_v1`) actually sends via WhatsApp today; attempts 2-4 will silently fail or send nothing meaningful until this is done. Email attempts 2-4 already work.
9. **Live-test `web/customers.html` and `web/settings.html`** — sign in, confirm the customer list loads and the status dropdown correctly updates `opt_out`/`do_not_call`, and confirm Settings loads/saves `businesses` contact fields + status correctly. Still not confirmed as of this session.
10. Formalize the ad-hoc `ALTER TABLE` changes (`review_request_link`, `project_image_url`) into tracked migration files if full reproducibility from `supabase/migrations/` matters later.
11. **Visual design pass on the web app** (colors, layout, etc.) — user has explicitly deferred this; functionality is prioritized first.
12. Optional: build a self-service "invite staff" flow on `settings.html` so linking additional users to a business doesn't require a manual SQL insert into `business_members` (see "Unassigned users, linking to a business" section above).
