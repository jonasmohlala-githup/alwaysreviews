...
- **Customer**: Veronica Mohlala (originally "Thabo Nkosi", renamed), `id = 28fe3adc-adff-49f9-9369-166431533dbd`
  ...
  - phone `27608491313`
- **Activity**: one "Request Sent" / WhatsApp / Sent row, `id = 0d997886-7516-4d94-9629-c1cf756c8523`, plus two older "Follow-up" rows with reviews attached (real Google reviews pulled in by the inbound-GBP workflow, not from an outbound send — confirmed with the user 2026-08-24, not a dashboard bug despite the resulting "0 requests sent, 2 reviews received, 0% conversion" stat display)
- **Auth user**: `jonas.mohlala@gmail.com`, `id = 0b146b20-42a0-4d63-a26e-afc507838c69`, linked to Print Express via `business_members` as `owner`, and (as of 2026-08-23/24) as `admin` on Print Express plus 10 demo businesses — see "Demo seed data" section further down for the full list and the business-switcher context.
...
## Custom domain connected — `alwaysreviews.co.za` (2026-08-31 session)

**Domain is `.co.za`, not `.com`** — the `.com` was never actually purchased; the user bought `alwaysreviews.co.za` via **GoDaddy** instead. Every `.com` reference in this file and in `marketing/always-reviews-gtm-strategy.html` has been corrected to `.co.za` (the marketing doc was also republished to its existing Claude Artifact URL). No `.com` references should remain anywhere in this project going forward — always check before adding new ones.

**GoDaddy has no API access on this account** (confirmed — no key/secret exists, account likely doesn't qualify for GoDaddy's Production OAuth tier) — DNS records had to be added manually via the GoDaddy dashboard, not automated. Records added (all confirmed propagated same-session via `nslookup 8.8.8.8`):
- `A @ → 76.76.21.21` (Vercel) — edited from GoDaddy's default parked "WebsiteBuilder Site" A record
- `CNAME www → alwaysreviews.co.za.` — was already present from GoDaddy's defaults, left as-is (resolves correctly once the apex A record points at Vercel, no need to point it at `cname.vercel-dns.com` directly)
- `TXT resend._domainkey → <Resend DKIM public key>` — for Resend domain verification
- `MX send → feedback-smtp.eu-west-1.amazonses.com` (priority 10) — Resend/SES bounce handling
- `TXT send → v=spf1 include:amazonses.com ~all` — Resend/SES SPF
- Pre-existing `_dmarc` TXT record (`v=DMARC1; p=quarantine; ...`) left untouched, unrelated to this setup, no conflict.

**Vercel**: both `alwaysreviews.co.za` and `www.alwaysreviews.co.za` added to the `alwaysreviews` project via `vercel domains add` (CLI, already authenticated from the earlier deploy session — no token needed). Vercel auto-provisioned SSL; both domains confirmed serving `200` over HTTPS via a live `curl` same-session. Nameservers intentionally stayed on GoDaddy's (`ns05`/`ns06.domaincontrol.com`) rather than switching to Vercel's (`ns1`/`ns2.vercel-dns.com`) — the A-record approach was used instead of nameserver delegation; both are valid, this just means DNS management stays in GoDaddy rather than moving to Vercel.

**Resend domain verification**: created via API (`POST /domains`, region `eu-west-1`) rather than the dashboard, using the existing `RESEND_API_KEY` from `.env`. All three DNS-dependent checks (DKIM, SPF, MX) came back `verified` within roughly a minute of the GoDaddy records being added — confirmed via `POST /domains/{id}/verify` then `GET /domains/{id}`. **This was the actual hard blocker mentioned throughout "Next steps" below — it is now resolved.**

**Supabase Auth reconfigured** via the Management API (`PATCH /v1/projects/xbmoomufmxicwymwbovg/config/auth`):
- `site_url` → `https://alwaysreviews.co.za`
- `uri_allow_list` → `https://alwaysreviews.co.za/dashboard.html,https://alwaysreviews.co.za/**,https://www.alwaysreviews.co.za/**,https://alwaysreviews.vercel.app/dashboard.html,https://alwaysreviews.vercel.app/**` (kept the old `.vercel.app` entries too, as a fallback — doesn't hurt to leave them)
- `smtp_admin_email` (the from-address) → `noreply@alwaysreviews.co.za`, replacing the shared `onboarding@resend.dev` test address

**Real bug hit and fixed while doing this — flagging for any future Supabase Auth config PATCH**: the Management API's `config/auth` PATCH endpoint does **not** merge partial updates into the existing SMTP block — sending only `{"smtp_admin_email": "..."}` silently **nulled out every other `smtp_*` field** (`smtp_host`, `smtp_port`, `smtp_pass`, `smtp_user`, `smtp_sender_name` all came back `null` in the response), which would have broken auth email sending entirely if left that way. Caught immediately by reading the PATCH response rather than assuming success. **Always send the full SMTP field set together in one PATCH when changing any single SMTP field** — host, port (as a **string**, e.g. `"465"` — the API rejects a JSON number with `"expected string, received number"`, a separate gotcha hit in the same fix), user, pass, sender_name, admin_email, all at once. Fixed by immediately re-sending all fields together; verified restored via a follow-up `GET`.

**Verified working end-to-end, same session**: sent a real magic-link OTP via `POST /auth/v1/otp` using the app's own anon key (`web/shared.js`), confirmed via Resend's send log (`GET /emails`) that it went out as `"Always Reviews" <noreply@alwaysreviews.co.za>` with `last_event: "delivered"` — not the old `onboarding@resend.dev` test address. Since the domain is now verified, this is no longer restricted to only the Resend account's own email (the test-mode limitation described earlier in this file, under "Important caveat, still open") — it can now deliver to **any** recipient, including real business staff/customers. **This unblocks onboarding a real second business**, which was previously gated on exactly this.

**Login flow click-tested end-to-end via Playwright, same session**: sent a real magic link, followed it immediately (per the project's standing testing convention above), landed on `https://alwaysreviews.co.za/dashboard` (redirected from `/dashboard.html` by Vercel's `cleanUrls: true`), fully signed in as `jonas.mohlala@gmail.com`, admin business switcher showing "Print Express", real dashboard data rendered (2 reviews received, 5.0 avg rating, both Follow-up activities from the seed data). Confirmed via screenshot. `web/` HTML files use relative redirect paths (`window.location.href = "dashboard.html"`), so no code changes were needed for the domain swap itself — this is now the fully verified, working custom domain.
...
## Demo seed data + send-review-request.json fixes (2026-08-24 session)

**10-business demo seed**: `supabase/migrations/20260824000000_seed_demo_businesses.sql` — unlike the earlier two-business demo (seeded, verified, then deliberately removed in the 2026-08-23 session), this one was seeded and **kept** live. 10 fake local-service businesses (Kimberley Auto Care, Bright Smile Dental, GreenLeaf Landscaping, Diamond City Plumbing, Northgate Hair Studio, Sunrise Electrical Services, Karoo Kitchen Restaurant, PawsCare Veterinary Clinic, Halfway House Fitness [Paused], Big Hole Bakery), 34 customers, 34 activity rows with a deliberately varied mix (5★/4★/3★ reviews received, pending/sent-no-review-yet, one Follow-up, one Failed send, one opted-out customer) so dashboard stats differ meaningfully business to business. All 10 linked to `jonas.mohlala@gmail.com` as `admin` via `business_members`, same pattern as the 2026-08-23 admin-role work — so the business-switcher dropdown now always shows 11 options (10 demo + Print Express).

**`send-review-request.json` — real bug found and fixed: nested `{{ }}` placeholder syntax broke n8n's expression parser.** The Gmail "Send a message" node's subject expression did `$json.email_subject.replace('{{business_name}}', ...)` — n8n's editor treats any `{{`/`}}` it sees as an expression boundary, even nested inside an outer `.replace()` string literal, so it misparsed this as a malformed second expression ("[ERROR: invalid syntax]", caught visually while inspecting the node in n8n's editor). **Fixed** by switching the placeholder format to single braces (`{business_name}`, `{first_name}`) in both the workflow expression and the actual stored data — `supabase/migrations/20260824010000_fix_email_placeholder_braces.sql` converts Print Express's existing `email_subject_1..4`/`email_intro_1..4` values from the old `{{...}}` format. Any future AI-generated subject/intro content must use single braces to match.

**Two validation Code nodes added** (`Validate WhatsApp Template`, `Validate Email Content`), inserted between the `If` node and the send nodes on each branch. These don't do any selection/switching themselves — the SQL query's `CASE` statements already did that, unchanged — they just throw a clear, specific error (naming the business, customer, and attempt number) if the SQL-resolved value comes back null, instead of silently sending `null` to Pabbly/Gmail and failing there with a worse error. Considered and rejected duplicating into 4 HTTP Request + 4 Gmail nodes (one per attempt) via a Switch node first — reverted after user feedback that it added structural duplication for no real benefit, since the SQL already resolves the right value per attempt before any node needs to branch on it.

**Real design correction: Pabbly's "API Campaign" broadcast type requires BOTH `templateName` AND `campaignName` per API call**, confirmed against the user's own working curl example (not assumed from the dashboard UI alone — an earlier wrong guess, based only on the "Select Template here" required-field screenshot, assumed `campaignName` alone controlled which template fired and swapped the two fields' roles; corrected once the real curl output showed both fields present together). Since each attempt (1-4) needs a different approved template, and Pabbly's dashboard ties one template to one Campaign Name at creation time, **each attempt also needs its own separate pre-created Pabbly campaign** — not just its own template. New columns `whatsapp_campaign_1..4` added (`supabase/migrations/20260824040000_whatsapp_campaign_columns.sql`), selected via the same per-attempt `CASE` pattern as `whatsapp_template_1..4` in the SQL query, and wired into the HTTP Request body as `campaignName: $json.whatsapp_campaign` (was hardcoded `"review"`). The `Validate WhatsApp Template` guard now also checks this field. Only `whatsapp_campaign_1` ("review") is a real, already-created Pabbly campaign as of this session — attempts 2-4 will fail at Pabbly until those campaigns are actually created there.

**Shared-value column defaults added, so onboarding a new business no longer needs 12 columns filled in manually**: `whatsapp_template_1..4`, `email_subject_1..4`, `email_intro_1..4`, and `whatsapp_campaign_1..4` all turned out to be the same values across every business (confirmed with the user, not assumed) rather than per-business custom content — so all 12 columns now have Postgres `DEFAULT` values matching Print Express's real content (`supabase/migrations/20260824020000`, `20260824030000`, `20260824040000`, `20260824050000`). A plain `insert into businesses (...)` omitting these columns now gets them for free. **Real caveat carried forward, not resolved by this**: `whatsapp_template_2/3/4` (`review_request_v2/3/4`) and `whatsapp_campaign_2/3/4` (`review_2/3/4`) are still placeholder names with no real Meta-approved content or real Pabbly campaigns behind them — the defaults just mean every business now has *consistent* placeholder data instead of some having it and some having null.

**Revised onboarding column list** (superseding any earlier version): only `business_name`, `contact_person`, `contact_email`, `contact_phone`, `gbp_account_id`, `location_id`, `whatsapp_sender_number`, and `review_request_link` need to be provided per business now — everything messaging-related is either a column default or (for email subject/intro) AI-generated at send time, not part of onboarding data at all.

**Full workflow accuracy pass performed** (live SQL execution against the real DB, structural graph checks for orphaned/unreachable nodes, cross-referencing every `$json.field` reference against the SQL's actual output columns, parameterized-query placeholder-count checks) — found two pre-existing real gaps, unrelated to this session's edits: (1) `review_request_link` is `null` for **every** business including Print Express — the email's "Leave a Review" button and Pabbly's `link` field would both go out broken; both Code node guards were extended to also catch this. (2) The Pabbly HTTP Header Auth credential is still the placeholder ID (`REPLACE_WITH_YOUR_PABBLY_CHATFLOW_CREDENTIAL_ID`) — already a known, previously-documented gap, not new.

**Messaging framework updated**: added section 08 "Value-add" to `marketing/always-reviews-messaging-framework.html` (republished to the same artifact URL) — positions a third-party embeddable Google-reviews widget (Elfsight/Trustmary/EmbedSocial, explicitly **not** something Always Reviews builds) as a pitch talking point, framed as "I'll help you get your reviews live on your own site" rather than "we include a widget." Sits after The Watch/The Reply in the feature ladder as a bonus reinforcing the core promise, not a competing headline.

## GBP verification gate + n8n workflow accuracy pass (2026-08-31 session)

**Real gap identified**: Google will not accept an automated reply (`Post Reply to GBP`) on a Business Profile that isn't verified with Google. Nothing in this project's GBP integration can detect verification status via API — it's a manual, one-time fact about the business's real Google account.

**`businesses.gbp_verified` column added**: `supabase/migrations/20260831000000_gbp_verified_column.sql` — `boolean not null default false`. Pushed live via `supabase db push --linked`. Set manually per business — not derived from any API call this project makes. **Print Express marked `true`** (confirmed verified by the user 2026-08-31) via direct SQL update; every other business (the 10 demo businesses) defaults `false`.

**`web/settings.html` updated**: added a "Google Business Profile is verified" checkbox (using the existing `.checkbox-row` CSS convention), wired to `businesses.gbp_verified` via the same `sb.from("businesses").update(...)` pattern as the existing status dropdown. **Not yet deployed to production** — `vercel deploy --cwd web --yes --prod` was blocked by the auto-mode safety classifier (a live-affecting publish action outside this session's pre-authorized scope) and still needs to be run.

**`inbound-gbp-review-capture.json` updated to gate replies on verification** (decision confirmed with the user via AskUserQuestion: skip only the reply, still capture the review and still notify the owner — not skip the whole business):
- `Get Active Businesses with GBP` now selects `gbp_verified`; `Flatten Reviews` carries it (coerced to a strict boolean) onto every review item.
- New **`GBP Verified?`** IF node inserted between `Already Replied on Google?`'s false branch and `Good Review? (4-5 stars)`. True → unchanged AI-reply flow. False → new **`Log Skipped Reply (Not Verified)`** node, which leaves `activity_status = 'Pending'` (not `Completed`) and appends a note, then joins the existing low-star notify path — so the owner still gets emailed, just without a posted reply.
- Because the skipped review stays `Pending`, it's automatically picked back up and retried by the existing `Needs Insert or Just Retry?` mechanism on the next scheduled run — the moment a business is marked verified, its backlog of skipped reviews gets real replies with no manual re-processing.
- `Notify Business of New Review`'s message logic gained a third case (alongside "already replied on Google" and "auto-replied"/"needs attention"): explains the reply was withheld specifically because the profile isn't verified yet, and that it'll happen automatically once it is.
- **User asked whether this should instead be split into two separate n8n workflows** (capture workflow calling a reply workflow via Execute Workflow) — considered and rejected: the existing `Pending`/retry pattern already gives capture and reply a database-mediated handoff that's more robust than an in-memory Execute Workflow call (survives partial failures, doesn't need both workflows active simultaneously, keeps one execution log per review for debugging). No concrete reason to split existed (no differing schedules, no reuse case). Confirmed with the user to keep it as one workflow.
- **Full accuracy pass performed on request before import**: JSON validity, connection-graph integrity (no dangling references, no unreachable nodes — 34 nodes total), `.item` reach-back safety re-verified for `Get Business Contact Email (from Low-Star Path)` across all 3 paths that now feed it (confirmed no Merge node crossed on any path, so the existing "safe only in strict 1-in/1-out chains" rule from earlier sessions still holds), parameterized query placeholder counts (all 5 Postgres nodes with `queryReplacement` — all matched), `gbp_verified` column nullability confirmed `NOT NULL DEFAULT false` (no null-handling edge case), `notes` column concatenation pattern confirmed safe (always non-null at insert time, consistent with 2 pre-existing nodes using the same pattern). No new bugs found — safe to import.

**Other pending fixes on this same file re-confirmed still present** (from earlier in this session, not yet re-imported): `.first()`/error-handling/SQL-injection fixes from the 2026-08-20 session are all still in the working tree, unchanged.

**`send-review-request.json`**: small additional fix this session — the Pabbly WhatsApp HTTP body's `bodyParams` array was missing `$json.review_request_link` entirely (only `[$json.first_name, $json.business_name]`), meaning the WhatsApp template's review-link variable would never have been populated even once the campaign/template setup is complete. Now `[$json.first_name, $json.business_name, $json.review_request_link]`.

**WhatsApp template names changed**: `supabase/migrations/20260825010000_rename_whatsapp_templates.sql` — `review_request_v1..4` turned out to be unavailable in Meta Business Manager (already registered elsewhere), so every business's `whatsapp_template_1..4` (and the column defaults) were renamed to `alwaysreviews_ask_v1..4`. **Every earlier reference to `review_request_v1..4` in this file and elsewhere is now stale** — the real gap (only attempt 1 has actual Meta-approved content, 2-4 are still placeholder names) is unchanged, just under new names.

**Print Express's `review_request_link` finally set**: `supabase/migrations/20260825000000_set_print_express_review_link.sql` — was `null` since the column was added (see the 2026-08-24 accuracy-pass note above); now `https://g.page/r/CfoxDhgwSOL5EBM/review`. Closes punch-list item 7a **for Print Express only** — still `null` for the 10 demo businesses (not real, no action needed there).

**Test auth account cleanup**: `printexpressnamakwa@gmail.com` (a real Print Express customer's email, used for auth testing since 2026-08-18 per earlier session notes) was signed into again this session to verify the new domain's email delivery works for genuinely unrecognized recipients (it does — confirmed `delivered` via Resend, and the app correctly showed "not linked to a business yet" with no data leak). Deleted afterward at the user's request via `delete from auth.users where id = '...'` through the Management API (the GoTrue admin REST endpoint rejected the Supabase access token — needs the actual service-role key, not tried, since deleting via SQL was simpler and worked immediately). No `business_members` row existed, so nothing else needed cleanup.

**Revised onboarding column list** (supersedes the 2026-08-24 version): `business_name`, `contact_email` (practically required — where notifications go), `gbp_account_id`, `location_id` (both required — without them the business is silently excluded from GBP review capture entirely), `review_request_link` (required — validation guards in `send-review-request.json` block sends without it), plus `contact_person`/`contact_phone`/`whatsapp_sender_number` (recommended, not blocking). **`gbp_verified` added as a new post-onboarding step**: leave `false` at onboarding, flip to `true` in Settings only once verification with Google is actually confirmed — not part of the initial data entry, since it's rarely known at the moment a business is first added.

## Magic-link login loop fix + mobile hamburger nav + read-only GBP checkbox (2026-08-31 session, continued)

**Bug: clicking the magic link redirected back to the login page instead of signing in.** Root cause: `getSession()` in `web/shared.js` called `sb.auth.getSession()` immediately on page load, but after a magic-link redirect the session token sits in the URL hash (`#access_token=...`), which `supabase-js` parses asynchronously right after the client initializes. `dashboard.html`'s auth check was racing that parse — it would frequently see no session yet and bounce straight back to `index.html`, discarding the token in the process. All four authenticated pages (`dashboard.html`, `add-customer.html`, `customers.html`, `settings.html`) share this same `getSession()` helper, so all were equally affected even though only the dashboard case was reported.

**Fix**: `getSession()` in `shared.js` now waits for the first `onAuthStateChange` event (fires once per page load, session or not) before falling back to a direct `getSession()` call — guarantees the hash has already been processed. Verified live end-to-end via Playwright (per this project's standing testing convention): sent a fresh magic link to `jonas.mohlala@gmail.com` with `emailRedirectTo` pointed at the production domain, followed it immediately, landed cleanly on the dashboard signed in, business switcher and real activity data rendered, no loop. Deployed via `vercel deploy --cwd web --yes --prod`.

**Mobile hamburger nav added**: below 720px, `.topbar`'s nav links + business switcher + sign-out were wrapping/jumbling together in a single flex row with no responsive handling at all (no media queries existed in `styles.css` prior to this). Added `.nav-toggle` hamburger button + `@media (max-width: 720px)` rules in `styles.css`, and `initMobileNav()` in `shared.js` (self-invoking on `DOMContentLoaded`, injects the toggle button into any page's `.topbar` and toggles a `.nav-open` class) — one shared implementation picked up automatically by all four authenticated pages, no per-page JS needed. Verified visually via a standalone Playwright harness at a 375px viewport (closed state: brand + hamburger only; open state: nav links stacked, switcher/email/sign-out full-width below a divider) before deploying. Live on production.

**Settings: "Google Business Profile is verified" checkbox made read-only**: was previously an editable checkbox businesses could toggle themselves, which defeats the point — GBP verification is confirmed by Always Reviews (the admin), not self-reported (see the `gbp_verified` column work earlier this session). `web/settings.html`'s `#gbpVerified` checkbox now has the `disabled` attribute (still shows the real stored value, just can't be changed) and was dropped from the form's `sb.from("businesses").update(...)` payload entirely, so it can no longer be flipped from this page even via a crafted request. Copy updated to say "contact us once your profile is verified with Google" instead of implying self-service. Deployed.

## Consent checkbox removed from Add Customer form (2026-08-31 session, continued)

**`web/add-customer.html`**: removed the "This customer has agreed to be contacted about leaving a review" checkbox — user's explicit preference is that adding a customer implicitly asserts consent, no separate confirmation step. The form now always submits `consent_optin: true` in the webhook payload (was `document.getElementById("consent").checked`). Verified the n8n side needs no change: `customer-intake-form.json`'s "Prepare Filename" node already does `consent_optin: !!body.consent_optin` (passthrough, no default-false logic to fight) and the parameterized insert forwards it unchanged — so `true` flows straight into `customers.consent_optin` with no additional wiring. Deployed to production.

**Follow-up: DB column default also flipped to match** — the user clarified the intent explicitly: `consent_optin` should be `true` by default until a customer opts out (`opt_out = true`, set via `web/customers.html`'s status dropdown), at which point `send-review-request.json`'s eligibility query (`where c.opt_out = false and c.do_not_call = false and c.consent_optin = true and ...`) excludes them. This was already the *effective* behavior via the website's hardcoded `true`, but the underlying `customers.consent_optin` column itself still defaulted to `false` (set in the original `20260813000000_init.sql`) — meaning any other insert path (manual SQL, a future integration) would have silently gotten `false` and been excluded from review requests with no obvious error. Fixed via `supabase/migrations/20260831020000_consent_optin_default_true.sql` (`alter table customers alter column consent_optin set default true`), pushed live and verified (`information_schema.columns` confirms `column_default: true`). Net effect, confirmed correct: consent is true by default everywhere now (website + schema), and `opt_out` is the only lever that removes a customer from eligibility.

## Field launch starting (2026-08-31 session, continued)

User is starting in-person visits to local businesses in the area to pitch Always Reviews, starting the day after this session. Deferred a pre-launch hardening pass (re-import all 3 n8n workflows with accumulated fixes, create the real Pabbly HTTP credential, sort out WhatsApp attempts 2-4) to do **tomorrow (2026-09-01)** rather than tonight — "we'll do that tomorrow." Until that pass happens: only WhatsApp attempt 1 (`alwaysreviews_ask_v1`) and email (all 4 attempts) are real working send paths; anything beyond that for a genuinely new real client should wait on tomorrow's session. See "Next steps" list below for the specific items to work through.

## Public landing page + login moved off the root (2026-08-31 session, continued)

**Built `marketing/always-reviews-landing.html`**: a real public marketing page, warm-paper/gold system matching the other 3 marketing docs (Fraunces/Source Sans 3/IBM Plex Mono), built from the messaging framework's exact positioning line, headline options, and the Ask/Watch/Reply/Proof feature ladder — nothing invented that wasn't already decided in that doc. Hero includes a "proof panel" styled as a browser chrome around Print Express's real dashboard stats (2 reviews, 5.0 avg, 4 automatic follow-up attempts, 0 manual) — no fabricated numbers. **User explicitly said to leave out pricing and any social-proof/testimonial framing for now** (single real client is too thin to lean on as trust-building copy) — cut a planned "Trust" section and review-quote cards from the hero panel accordingly; revisit once there's a small roster of real clients. WhatsApp CTA buttons wired to `wa.me/27795079693` with a pre-filled starter message.

**Real routing bug found and fixed**: `alwaysreviews.co.za/` was serving `web/index.html`, which was the **login page** — meaning every cold visitor hit a sign-in prompt with no public page to browse at all. Fixed by restructuring: the login page's content moved to new `web/login.html`, and the landing page above became the new `web/index.html`. All 12 `window.location.href = "index.html"` redirects across `dashboard.html`, `add-customer.html`, `customers.html`, `settings.html` (3 each: no-session redirect, sign-out handler, `SIGNED_OUT` auth-state listener) updated to point at `login.html` instead. Added a quiet "Sign in" link in the landing page's nav for existing customers. Confirmed Supabase's `uri_allow_list` needed no change (references `dashboard.html`/wildcards only, never `index.html`). Verified locally and on production via Playwright: `alwaysreviews.co.za/` now renders the landing page directly with no auth redirect, `/login` still serves the sign-in form correctly. Deployed.

## Follow-up bug from the routing change: magic link fell back to the landing page (2026-08-31 session, continued)

**Real bug, direct consequence of the index.html/login.html swap above**: after moving the public landing page onto the root, magic links stopped reliably landing on the dashboard — they'd land back on `index.html` (now the static landing page, with zero auth logic) instead. Root cause: Supabase's `site_url` was still `https://alwaysreviews.co.za` (the bare domain) — previously harmless because that URL used to be the login page (which forwards to `dashboard.html` itself if a session exists), but now broken because the same URL resolves to the landing page, which has no session check or forwarding at all. Any redirect that falls back to `site_url` (rather than respecting the explicit `redirect_to` param on the link) now dead-ends there. **Fixed** via the Management API: `site_url` → `https://alwaysreviews.co.za/dashboard.html`. `uri_allow_list` and SMTP fields confirmed untouched by the PATCH (checked explicitly, given the earlier session's discovery that a partial PATCH to this endpoint can silently null unrelated SMTP fields). Verified live via a fresh magic link + Playwright, same pattern as this project's standing testing convention: landed cleanly on `alwaysreviews.co.za/dashboard`, signed in, real data rendered, no more falling back to the landing page.

## Sign-out now redirects to the landing page, not login (2026-08-31 session, continued)

**User request**: after signing out, land on the public landing page (`index.html`) rather than the login form — the login page is for someone trying to get back in, not someone who just left. Implemented as a split: the **no-session redirect** (page loads with no valid session at all — expired/missing token) still goes to `login.html`, since that's someone trying to get back in; only the **explicit "Sign out" button click** now goes to `index.html`. Confirmed with the user via AskUserQuestion before implementing — this was the recommended and chosen option over sending both cases to the landing page.

**Real race condition found and fixed along the way**: each page also had a `sb.auth.onAuthStateChange` listener redirecting to `login.html` on the `SIGNED_OUT` event — but `signOut()` fires that event *internally*, before `await signOut()` even resolves in the click handler. So the click handler's new `index.html` redirect and the listener's `login.html` redirect were racing on every sign-out click, with the outcome dependent on browser microtask timing rather than being deterministic. Confirmed via grep that `signOut()` is only ever called from each page's own click handler (no other code path fires `SIGNED_OUT`), meaning the listener was purely redundant, not covering a genuinely different scenario — removed the `onAuthStateChange`/`SIGNED_OUT` listener entirely from all four pages (`dashboard.html`, `add-customer.html`, `customers.html`, `settings.html`), leaving exactly one deterministic redirect per case (no-session → `login.html`, explicit sign-out → `index.html`). Verified live: sent a real magic link, signed in via Playwright, clicked "Sign out," confirmed landing cleanly on `alwaysreviews.co.za/` (the landing page, "Sign in" + "See it live" in nav) with no bounce to the login form. Deployed.

## Git status as of end of 2026-08-31 session

**Nothing from this entire session has been committed yet** — user confirmed explicitly they'll commit via the CLI themselves rather than have it done here. Everything described above (magic-link race fix, mobile hamburger nav, read-only `gbp_verified` checkbox, consent-default changes + migration, the new landing page, the `index.html`/`login.html` routing split, the `site_url` Supabase fix, and the sign-out redirect change) is saved to disk and already deployed live to `alwaysreviews.co.za` via `vercel deploy`, but the git working tree is still dirty. Run `git status` at the start of any future session before assuming a clean baseline — as of this note, untracked/modified files include `web/login.html` (new), `marketing/always-reviews-landing.html` (new), 4 new migration files, and modifications across `web/*.html`, `web/shared.js`, `web/styles.css`, both n8n workflow JSON files, and this context file itself.

## Review-request emails switched from Gmail OAuth to Resend (2026-08-31 session, continued)

**Problem**: `send-review-request.json`'s email branch ("Send a message") sent outbound review-request emails to customers via a **Gmail OAuth credential connected to Jonas's personal `jonas.mohlala@gmail.com`** — meaning every customer's review-request email would arrive "from" his personal address, not the business. Confirmed directly with the user this was the actual live wiring, not an assumption.

**Fix**: converted the node from `n8n-nodes-base.gmail` to a generic `n8n-nodes-base.httpRequest` node (same pattern already used for the WhatsApp/Pabbly send — `POST https://chatflow.pabbly.com/...` via `httpHeaderAuth`), now named **"Send Email (Resend)"**, posting to `https://api.resend.com/emails`. Chosen over Google Workspace or a forwarding-only address because **Resend is already fully verified for `alwaysreviews.co.za`** (DKIM/SPF/MX all confirmed `verified` earlier this session — see "Custom domain connected" above) and already in live use for the app's own magic-link auth emails — no new service, no new domain verification step, same `RESEND_API_KEY` already sitting in `.env` (confirmed present, `re_XS6...` prefix, without printing the full key).

- **From address**: `Always Reviews <reviews@alwaysreviews.co.za>` — a different local-part than the auth emails' `noreply@alwaysreviews.co.za`, but Resend/SES domain verification covers the whole domain, not a single address, so no additional verification is needed for this new from-address.
- **Same HTML email body and subject-line placeholder logic as the original Gmail node** — copied over unchanged (project photo image, greeting, intro, "Leave a Review" button, sign-off). This is a sending-mechanism swap only, not a copy or design change.
- **Real gotcha avoided deliberately**: the HTML body's `${...}` JS template-literal interpolation was used instead of nested `{{...}}`, specifically because this exact project already hit a bug on 2026-08-24 where nested `{{ }}` inside a `.replace()` call broke n8n's expression parser (see that session's notes on `email_subject_1..4`/`email_intro_1..4` needing single-brace `{business_name}`/`{first_name}` placeholders). Verified before considering this done: extracted the raw JS expression and ran it directly in Node with sample data, confirmed valid JSON output and correct placeholder substitution; also confirmed the file's connection graph has no dangling references after the node rename (`"Send a message"` → `"Send Email (Resend)"` updated in the `connections` block too).
- **New credential needed in n8n before this can run**: `resendApi`, Header Auth, header name `Authorization`, value `Bearer <RESEND_API_KEY from .env>` — currently a placeholder ID (`REPLACE_WITH_YOUR_RESEND_CREDENTIAL_ID`), same placeholder-credential convention already used for `pabblyChatflowApi`. Not yet created.
- **Not yet live-tested** — this workflow is still `active: false` and still has its full existing list of pending fixes bundled in (IF-branch swap, validation nodes, per-attempt template selection, etc. — see the "Re-import ALL THREE n8n workflows" punch-list item below). A real send test (does Resend actually deliver this from-address, does the HTML render correctly in a real inbox) is still outstanding.
- **`reviews@alwaysreviews.co.za` deliberately kept send-only, no forwarding set up** — user considered and explicitly decided against GoDaddy email forwarding for this address (unlike the earlier discussion about a general-purpose mailbox). Since a customer could plausibly reply to a review-request email (a question, a complaint) and that reply would otherwise silently vanish, added a small muted footer line to the email body: *"This inbox isn't monitored — for any questions, please contact {business_name} directly."* — sets the right expectation and redirects real replies back to the actual business instead of into a void. Verified the JSON is still valid and the JS template-literal still evaluates correctly after this edit.

**Second Gmail node found and converted the same session**: `inbound-gbp-review-capture.json`'s **"Notify Business of New Review"** node — the email that alerts a business owner every time a new Google review comes in (all 4 message-branch variants: already-replied-on-Google, not-yet-verified, good-review-auto-replied, low-star-needs-attention) — was also a `n8n-nodes-base.gmail` node on the same personal Gmail OAuth credential. Converted the same way as `send-review-request.json`: now an HTTP Request node to `https://api.resend.com/emails`, using the same `resendApi` Header Auth credential (placeholder ID, shared across both workflows — create once in n8n, attach to both nodes). **From-address deliberately kept distinct**: `notify@alwaysreviews.co.za` here (business-facing) vs. `reviews@alwaysreviews.co.za` on the other workflow (customer-facing) — user's explicit choice, for clarity/filterability rather than reusing one address for both purposes. Verified: valid JSON, no dangling connection references (node name unchanged, only its internal type/parameters/credentials changed), and the JS template-literal body tested directly in Node across all 4 message branches with realistic sample data — all produced valid JSON with correctly substituted content. Neither workflow has any remaining `gmail`/`Gmail` reference at all (confirmed via grep) — every outbound email in this project now goes through Resend.

## Next steps (pick up here)

Updated 2026-08-31. Done since the last update: **custom domain `alwaysreviews.co.za` connected end-to-end** — Vercel domain added + SSL live, GoDaddy DNS records added, Resend domain verified (DKIM/SPF/MX all `verified`), Supabase `site_url`/`uri_allow_list`/SMTP sender address all updated, real magic-link send confirmed `delivered` from `noreply@alwaysreviews.co.za` via Resend's log. See "Custom domain connected" section above for full detail. Items 1 and 2 below are now done — kept struck through for history, renumbered list follows.

~~1. Verify `alwaysreviews.com` on Resend and switch the SMTP from-address off `onboarding@resend.dev`~~ — ✅ done 2026-08-31 (domain is actually `.co.za`, not `.com`).
~~2. Connect a custom domain once purchased~~ — ✅ done 2026-08-31.

~~1. Live-test the login flow in a real browser on the new domain~~ — ✅ done 2026-08-31, same session (Playwright, screenshot confirmed).

Remaining open items, roughly in priority order:

1. ~~Deploy `web/` to production~~ — ✅ done 2026-08-31, multiple times over the course of the session (the `gbp_verified` checkbox, mobile nav, consent default, landing page, routing split, and sign-out redirect were each deployed individually as they were finished). `web/` is fully live and current as of the last deploy in this session — no local-only web changes remain outstanding.
2. **Re-import ALL THREE n8n workflows** — every workflow had real bugs fixed across sessions and none have been re-imported/re-tested since:
   - `inbound-gbp-review-capture.json`: `.first()`/error-handling/SQL-injection fixes from 2026-08-20, **plus the new `GBP Verified?` gate added 2026-08-31**, **plus the same session's Gmail→Resend switch on "Notify Business of New Review"** (see sections above) — needs the same `resendApi` credential as `send-review-request.json` (create once, attach to both). Re-test recommended once live, especially the new not-verified branch, the "Already Replied on Google?" branch, and the new Resend notify email (none were exercised live).
   - `send-review-request.json`: **the WhatsApp/email IF-branch swap is a real functional bug fix that needs to go live**, plus the 2026-08-24 session's fixes (single-brace placeholder format, the two validation Code nodes, dynamic `whatsapp_campaign`/Pabbly campaignName), the 2026-08-31 `bodyParams` fix (was missing `review_request_link` entirely), **and the same session's Gmail→Resend email-sending switch** (see "Review-request emails switched from Gmail OAuth to Resend" above — needs the new `resendApi` Header Auth credential created in n8n before import, same as the still-outstanding `pabblyChatflowApi` one) — currently `active: false`, so it's not silently misrouting customers or sending broken content right now, but don't activate it before re-importing all of this.
...
3. **Connect GitHub auto-deploy in Vercel** — currently deploys are manual (`vercel deploy --cwd web --yes --prod`); `vercel link`'s attempt to auto-connect the GitHub repo failed and likely needs the Vercel-for-GitHub App authorized via the dashboard first.
4. **Rotate exposed credentials before onboarding real client data** — Supabase secret/service-role key, Gemini API key, Pabbly Chatflow API key, and the Postgres database password were all at some point visible in a chat transcript or n8n export. This is now higher priority than before (real name, real domain plan, live public deployment).
5. **Fix the remaining raw-SQL-interpolation node**: "Log Reply on Activity" in `inbound-gbp-review-capture.json` still builds its query via string interpolation (lower risk than the ones already fixed, since it only touches `activity_id`, a UUID from our own database — not public review text — but same pattern class).
6. **Write and submit real WhatsApp template content for attempts 2-4, AND create the matching Pabbly campaigns** — templates are now named `alwaysreviews_ask_v2/3/4` (renamed 2026-08-25 from `review_request_v2/3/4`, which were unavailable in Meta Business Manager) and campaigns `review_2/3/4` — currently just placeholder names in the database with nothing real behind either one. Only attempt 1 (`alwaysreviews_ask_v1` template, `review` campaign) actually works today. The 2026-08-24 session's validation guards will now correctly block a send with a clear error naming the missing attempt, rather than letting a broken send reach Pabbly — but the underlying setup (write + Meta-approve 3 templates, create 3 Pabbly campaigns each with its template selected) is still not done. Email attempts 2-4 already work.
6a. ~~Set a real `review_request_link` for every business, including Print Express~~ — ✅ done for **Print Express only** 2026-08-25 (`https://g.page/r/CfoxDhgwSOL5EBM/review`). Still `null` for the 10 demo businesses (not real, no action needed).
6b. **Create the real Pabbly HTTP Header Auth credential in n8n** and attach it to `send-review-request.json`'s "HTTP Request" node — still the placeholder ID `REPLACE_WITH_YOUR_PABBLY_CHATFLOW_CREDENTIAL_ID`.
7. **Live-test `web/customers.html` and `web/settings.html`** — sign in, confirm the customer list loads and the status dropdown correctly updates `opt_out`/`do_not_call`, and confirm Settings loads/saves `businesses` contact fields + status correctly. The `gbp_verified` checkbox itself is now read-only (admin-set only, see section above) and confirmed rendering the correct stored value via the dashboard login test — but the rest of Settings' save flow (contact fields, status dropdown) and all of `customers.html` are still not confirmed as of this session.
8. Formalize the ad-hoc `ALTER TABLE` changes (`review_request_link`, `project_image_url`) into tracked migration files if full reproducibility from `supabase/migrations/` matters later.
9. **Visual design pass on the web app** (colors, layout, etc.) — user has explicitly deferred this; functionality is prioritized first.
10. Optional: build a self-service "invite staff" flow on `settings.html` so linking additional users to a business doesn't require a manual SQL insert into `business_members` (see "Unassigned users, linking to a business" section above).
11. **`gemini-2.5-flash-image` (used in `customer-intake-form.json`'s "Personalize Image with Gemini") is currently active/not deprecated** per Google's own docs, checked 2026-08-20 — but one unverified third-party source claimed an October 2026 shutdown date. Not acted on since unconfirmed against official docs, but worth a quick re-check of `ai.google.dev/gemini-api/docs/models/gemini-2.5-flash-image` before relying on it long-term.
12. **Confirm `gbp_verified` for the remaining live businesses** — Print Express is the only one marked `true` so far (confirmed by the user 2026-08-31); the 10 demo businesses are fake so don't need real verification, but any future real client's `gbp_verified` should be checked and set during onboarding, not left at the `false` default indefinitely.
